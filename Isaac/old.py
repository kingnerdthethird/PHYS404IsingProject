...
for parameter in parameters:
    row, col = parameter[0], parameter[1]

    spin_matrices, pretty_matrices, m_values, Ns, averages = [], [], [], [], []

    for i in range(0, N):
        spin_matrix, pretty_matrix, m = ising.GenerateMatrix(row, col)

        spin_matrices.append(spin_matrix)
        pretty_matrices.append(pretty_matrix)
        m_values.append(m)
        Ns.append(i + 1)
        average = ising.AverageMangnetization(m_values, i + 1, debug)
        averages.append(average)

    
    ising.PrintMatrices(spin_matrices, m_values, run_number, N, row, col)
    ising.PrintMatrices(pretty_matrices, m_values, run_number, N, row, col)
    print(m_values)
    # average = ising.AverageMangnetization(m_values, N, debug)
    ising.PlotMatrixData(Ns, [["Average", averages, [0] * N, plot_averages_on]], row, col, run_number, debug)
